...
| US-4.3 | Replace content deactivates the old lesson, links to the new one, leaves existing completions untouched | PASS | Code read (`admin-upload.tsx:204-227`) matches the AC exactly: new row inserted with `replaces_lesson_id`, old row updated `is_active=false, superseded_by_lesson_id=<new>`, two audit rows written (`upload` with `previous_lesson_id`, `deactivate` with `reason: superseded`). Exercised live against the running app (T37) — confirmed working. |
| US-4.4 | Reactivate restores a deactivated (non-superseded) lesson; superseded lessons never show Reactivate | PASS | Code read (`admin-upload.tsx:252-264, 543`) confirms the exact gating: `is_active` lessons get Edit/Replace/Deactivate; inactive+superseded get neither; inactive+not-superseded get Reactivate only. Exercised live against the running app (T37) — confirmed working. |
| US-4.5 | Edit details updates metadata only, never package_id/launch_path | PASS | Code read (`admin-upload.tsx:281-319`) only ever updates `title`/`description`/`duration_minutes`; `package_id`/`launch_path` aren't in the update payload. Exercised live against the running app (T37) — confirmed working. |
