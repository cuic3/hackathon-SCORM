...
| US-2.7 | A prospective learner with no existing account | Submits the signup form with a valid email/password and picks an existing seeded organization | A new learner profile is created against that organization and they can subsequently log in |
...
**Scope addition — learner self-service signup (confirmed with Claire Cui, 2026-08-18):** `/signup` is not exposed to the public — it is only used to create synthetic/seeded learner accounts against an existing seeded organization, the same category of data §4 already calls synthetic/seeded. It does not conflict with §4's rule; it's a path for producing that same seeded data, not a public-registration feature. This is grouped under US-2 because it exists to get a learner into the "complete a lesson" flow, not as a standalone feature. Also verified end-to-end: the live Supabase project requires email confirmation, and the confirmation email is delivered and the signup → confirm → login path works.
...
- **MUST**: all org/user/Elsevier-lesson data is synthetic/seeded. (§3.2/US-2.7: learner self-service signup is not public-facing and only ever produces synthetic/seeded accounts against a seeded organization — confirmed consistent with this rule, not an exception to it.)
...
- [x] A learner signs up via self-service signup (§3.2, US-2.7) → email confirmation IS required by the live Supabase auth settings; verified end-to-end (Claire Cui) that the confirmation email is actually delivered and the signup→confirm→login path works, so this is not a blocker for the demo.
...
## 9. Verify Table
Filled in this session (2026-08-18) by re-testing every AC against the running system: live Supabase project data (`list_tables`/`execute_sql`), the Vitest suite (`npm test` — 200/200 passing as of T42's source-institution-pill tests), `tsc -b --noEmit` (clean), and direct source review against the exact wording/behavior each AC requires. This table is scoped to the original founding ACs only (US-1.1/1.2, US-2.1/2.2, US-3.1/3.2/3.3, US-4.1); every other AC — including later additions like US-3.6's CSV export and US-4.3/4.4/4.5's content lifecycle — lives in §9.1. No interactive browser-automation tool was available for this session's own re-testing, so nothing below claims a fresh click-through login→launch→complete pass that didn't happen — rows verified that way say so in Notes; everything else is evidence from real historical usage already sitting in the live DB, or from reading the code that implements the AC.

| AC | Scenario | Verdict | Notes |
|---|---|---|---|
| US-1.1 | Valid SCORM 1.2 ZIP becomes a custom lesson | PASS | Live DB has 3 active custom lessons created via real admin uploads (`content_audit_log` shows 5 real `upload` actions total, including the T37 replace-content test), each with `package_id`/`launch_path` set. Code path (`admin-upload.tsx`) matches. |
...
| US-3.2 | Custom origin is marked in report | PASS | `report.tsx:139-141` renders the "Custom content"/Elsevier badge per row off `row.origin`. Confirmed by source read, not a fresh screenshot. |
...
| US-4.1 | Recorded completion survives content change | PASS | Confirmed end-to-end with a real live example (from T37's Replace content test): "Golf Explained" is now deactivated/superseded, and its `lesson_completions` row (`status='completed'`, `score_raw=100`) is still fully intact and unchanged — not just a structural guarantee anymore. Also backed structurally: `report.tsx` reads only denormalized snapshot columns (never live-joins `lessons`), the `lesson_completions.lesson_id` FK is `ON DELETE RESTRICT`, and there is no delete RLS policy on `lesson_completions` at all. |

### 9.1 Additional ACs (added during spec refinement)
This table holds every AC outside the original founding set (US-1.1/1.2, US-2.1/2.2, US-3.1/3.2/3.3, US-4.1 — kept in §9 above), regardless of whether it has a real §3 Given/When/Then row. Most now do (US-2.5/2.6/2.7 in §3.2, US-3.6 in §3.3, US-4.3/4.4/4.5 in §3.4); a few older ones (US-1.3/1.4/1.5, US-2.3/2.4, US-3.4/3.5, US-4.2) never got backfilled into a §3 row and are grandfathered here — not worth backfilling on this clock. Table is ordered numerically by AC ID.
...
| US-2.5 | Learner's own view shows "No score reported" (no "Completed —") for a finished lesson with no score | PASS | `format-score.ts`'s `formatScoreCell({audience:'learner'})` returns exactly `"No score reported"`, matching spec text verbatim; covered by `format-score.test.ts` (in the 200 passing tests). Live data has a real qualifying row (Lee Learner, "Infection Control Documentation", `status='completed'`, `score_raw=null` — the same row cited in US-3.5) that would render this text on `lesson.tsx`/`lesson-card.tsx`; not re-confirmed via a live browser session this session, so this is code+data evidence, not a fresh screenshot. |
...
| US-4.2 | Backend audit log records content changes, not exposed in report | PASS | `content_audit_log` is referenced only from `admin-upload.tsx` (and its test) — never from `report.tsx`/`home.tsx`. Live log has 5 `upload` + 3 `deactivate` + 1 `reactivate` + 1 `edit` rows from actual admin actions (including T37's live exercise of all three lifecycle actions), confirming the log is actually being written, not just designed. |
