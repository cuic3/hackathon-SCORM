...
## 9. Verify Table
Filled in this session (2026-08-18) by re-testing every AC against the running system: live Supabase project data (`list_tables`/`execute_sql`), the Vitest suite (`npm test` — 198/198 passing as of T40's CSV export tests), `tsc -b --noEmit` (clean), and direct source review against the exact wording/behavior each AC requires. Since this table was first filled in, US-4.3/4.4/4.5 were exercised live for real (T37) and US-3.6 (CSV export) was added (T40/T41, including a real-browser Playwright e2e test) — both are reflected below with their actual live evidence, not just code review. No interactive browser-automation tool was available for the rest of this session's own re-testing, so nothing below claims a fresh click-through login→launch→complete pass that didn't happen — rows verified that way say so in Notes; everything else is evidence from real historical usage already sitting in the live DB, or from reading the code that implements the AC.
