...
- [x] §2.10 (learner self-service signup) is built and confirmed in-scope (`spec.md` §3.2, US-2.7) — not public-facing, only produces synthetic/seeded accounts.
...
7. **Learner self-service signup** (§2.10): built and confirmed in-scope (spec.md §3.2, US-2.7) — DONE. The `profiles`-row-creation trigger on `auth.users` is still unverified against the live project.
...
- `cd app && npm test` — the Vitest suite (16 files, 200 tests as of T42's source-institution-pill tests). **Node version gotcha:** this machine's default `node` (Homebrew, on `PATH`) is v20.x; the test tooling (`jsdom`'s `undici`) requires Node ≥22.14/≥24 and hard-crashes on 20 with `webidl.util.markAsUncloneable is not a function`. Node 24 is available via `nvm`, but `nvm use 24` alone doesn't win against Homebrew's earlier `PATH` entry — run `export PATH="$HOME/.nvm/versions/node/v24.18.0/bin:$PATH"` first if `npm test` fails this way.
...
- Self-service signup (§2.10) is confirmed in-scope (spec.md §3.2, US-2.7) and email confirmation was verified working end-to-end; the `profiles`-row-creation trigger on `auth.users` still needs confirmation against the live project.
